...
(function () {
  ...
  Subset.reset = function (models, options) {
    ...
    models_ids = _.pluck(models, 'id');
    options = options || {};

    // delete parent reseted models
    parent_models = _.reject(parent_models, function (model) {
        incoming_model = _.find(models, function (this_model) {
          return this_model.id == model.id;
        });

      if (!incoming_model || !incoming_model.cached) {
        return _.include(ids, model.id);
      } else {
        _.extend(model.get("parameters"), incoming_model.parameters);
        return false;
      }
    });

    parent_models_ids = _.pluck(parent_models, 'id');

    // insert parent reseted models
    _.each(models, function (model) {
      if (!_.include(parent_models_ids, model.id) || !model.cached) {
    ...
    if (options.reset_to_subset) {
      // use parent version of model
      models = _.map(models, function (model) {
        if (_.include(parent_models_ids, model.id) && model.cached) {
          parent_model = _.find(parent_models, function (this_model) {
            return this_model.id == model.id;
          });
          if (parent_model) {
            _.extend(parent_model.get("parameters"), model.parameters);
            model = parent_model;
          }
        }
        return model;
      ...
      Backbone.Collection.prototype.add.call(this, models, _.extend({silent: true, add_to_subset: true}, options));
      this.trigger('reset', this, _.extend({model_ids: xored_ids}, options));
    } else {
  ...
  Subset.add = function (model, options) {
    if (options.add_to_subset) {
      this_model = Backbone.Collection.prototype.add.call(this, model, options);
      if (options.add_to_subset_and_parent) {
        _.result(this, 'parent').add(model, options);
      }
      return this_model;
    } else {
